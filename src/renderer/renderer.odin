...
init :: proc(window: ^window.Window) -> bool {
	log.info("Initializing renderer")

	state.ctx = context
	state.window = window; state.window.on_resize = resize
	state.instance = wgpu.CreateInstance(); if state.instance == nil { log.fatal("WebGPU is not supported. If it's REALLY needed, open an issue to create an optional OpenGL backend."); return false }
	...
	on_adapter :: proc "c" (status: wgpu.RequestAdapterStatus, adapter: wgpu.Adapter, message: string, userdata1, userdata2: rawptr) {
		context = state.ctx
		if status != .Success || adapter == nil do log.fatalf("Could not request WebGPU adapter: [%v] %s", status, message)
	...
	on_device :: proc "c" (status: wgpu.RequestDeviceStatus, device: wgpu.Device, message: string, userdata1, userdata2: rawptr) {
		context = state.ctx
		if status != .Success || device == nil do log.fatalf("Could not request WebGPU device: [%v] %s", status, message)
		state.device = device 

		width, height := state.window->get_framebuffer_size()
...
resize :: proc "c" () {
	context = state.ctx
	state.config.width, state.config.height = state.window->get_framebuffer_size()
	wgpu.SurfaceConfigure(state.surface, &state.config)
}
...
frame :: proc() {
	...
	case .Error:
		// Fatal error
		log.errorf("WebGPU surface texture error; status=%v", surface_texture.status)
	}
